...
namespace {
    bool lambda_wrapper(
        boost::python::object lambda, const fostlib::text_body &body
    ) {
        return false;
    }
}
...
void iterate_pop3_mailbox_string_host(
    ...
    iterate_pop3_mailbox(fostlib::host(host), lambda, name, password);
}
void iterate_pop3_mailbox(
    ...
    fostlib::pop3::iterate_mailbox( host,
        boost::lambda::bind(lambda_wrapper, lambda, boost::lambda::_1),
        name, password );
}
