...
namespace {
    /*
        Because start_response can be called a number of times we need to process things in an odd order.
        The wsgi_mime instance could be replaced multiple times up until the point where the first response iterable
        returns a non-empty string. This is watched for with the start_response_illegal flag.
    */
    struct wsgi_mime : public mime {
        ...
        // We need to store the start_response closure here so that it can error when the illegal flag is set
        ...
        struct iterator_wrapper : public iterator_implementation {
            ...
            const_memory_block operator () () {
                if ( !sent_first ) {
                    ...
                    throw exceptions::not_implemented("wsgi_mime::const_iterator::operator () () -- subsequent parts");
            }
        };
    ...
    /*
        We simply aren't going to support this right now. It might be possible to add it in later on.
    */
    void write(boost::python::str) {
        throw exceptions::not_implemented("Thsi WSGI server does not support the write outputter");
    }
    /*
        This acts as a closure where we can store the response that start_response starts to build.
        Each time start_response is called it will replace the existing mime. This allows the Python
        application to replace the response headers etc. in things like its error handling. It can do this
        as many times as it likes during the initual application start up, or when it is generating the first
        (non empty) string output. Once it has delivered a string then it cannot call start_response again
        as we have already started to send data to the client.
    */
    struct wsgi_response : boost::noncopyable {
        ...
        boost::python::object start_response(boost::python::object status, boost::python::list headers) {
            if ( response.get() && response->start_response_illegal )
                throw exceptions::not_implemented("You cannot call start_response after your iterable returns its first non-empty string");
